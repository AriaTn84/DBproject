...
@csrf_exempt
def cancel_with_penalty(request):
    ...
    try:
        ...
        confirm_cancellation = data.get('confirm', False)
    ...
    try:
        ...
        if reservation.get('payment_status') != 'Completed':
            connection.rollback()
            return JsonResponse({'error': 'Cannot cancel a reservation for which payment is not completed.'},
                                status=402)
        ...
        penalty_percentage = 1 - refund_percentage
        ...
        if not confirm_cancellation:
            ...
            return JsonResponse({
                ...
                'ticket_cost': f"{ticket_cost:.2f}",
                'penalty_percentage': f"{penalty_percentage:.0%}",
                'refund_percentage': f"{refund_percentage:.0%}",
