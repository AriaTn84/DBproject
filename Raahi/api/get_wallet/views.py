...
def get_wallet(request):
    ...
    if redis_client:
        try:
            ...
            if cached_user:
                return JsonResponse({
                    'message': 'Wallet fetched from cache successfully',
                    'data': cached_user,
                    'source': 'Redis Cache'
                })
        except Exception as e:
    ...
    try:
        ...
        cursor.execute(
            "SELECT w.balance, u.first_name, u.last_name, w.last_updated FROM Wallet w JOIN Users u ON u.user_id = w.user_id WHERE w.user_id = %s",
            (user_id,))
